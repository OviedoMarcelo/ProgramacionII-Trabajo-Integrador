...
public class MenuHandler {
    ...
    try {
        ...
        EmpresaDeEnvio empresa = EmpresaDeEnvio.valueOf(
                scanner.nextLine().trim().toUpperCase());

        // Tipo
        System.out.println("Tipos disponibles: ESTANDAR, EXPRES");
        System.out.print("Tipo de envío: ");
        TipoDeEnvio tipo = TipoDeEnvio.valueOf(
                scanner.nextLine().trim().toUpperCase());

        // Estado
        System.out.println("Estados disponibles: PREPARACION, VIAJE, ENTREGADO");
        System.out.print("Estado: ");
        EstadoDeEnvio estado = EstadoDeEnvio.valueOf(
                scanner.nextLine().trim().toUpperCase());
        ...
        pedido.setFecha(LocalDate.now());           // fecha automática
        pedido.setEstado(EstadoDePedido.NUEVO);     // estado inicial del pedido
        pedido.setEliminado(false);                 // baja lógica
        ...
        envio.setFechaEstimada(LocalDate.now().plusDays(3)); // ejemplo
        ...
        System.out.println("✓ Pedido con envío creado exitosamente!");

    }catch (IllegalArgumentException e) {
        // Ocurre si el usuario escribe mal el ENUM
        System.out.println("⚠ Valor inválido. Debe coincidir con los valores mostrados.");
        
    }
        // Ocurre si el usuario escribe letras en un número
         catch (Exception e) {
